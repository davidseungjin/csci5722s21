function idx = HAClustering_3A(X, k, visualize2D)
...
% clusters to be the single-link inter-cluster distance.
%
% INPUTS
...
%           point X(i, :) has been assigned to cluster c.
    ...
    % The single-link distances between all of the clusters. Initially,
    ...
    while num_clusters > k
        ...
        % Find the pair of clusters that are closest together by single-link.
        ...
        % its cluster size to 0 and setting the single-link distance
        ...
        % Update the dists array using the minimum distance between points
        % of each cluster (single-link method).
